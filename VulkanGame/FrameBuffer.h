...
namespace vkInit {
	...
	*/
	...
	void make_framebuffers_animation(framebufferInput inputChunk, std::vector<vkUtil::SwapChainFrame>& frames, bool debug) {

		for (int i = 0; i < frames.size(); ++i) {

			std::vector<vk::ImageView> attachments = {
			frames[i].postProcessInputAttachment.view, frames[i].depthBufferView,
			};
			...
			try {
				frames[i].animationFramebuffer = inputChunk.device.createFramebuffer(framebufferInfo);
