...
vkInit::SwapChainBundle vkInit::create_swapchain(vk::PhysicalDevice physicalDevice, vk::Device logicalDevice, vk::SurfaceKHR surface, ivec2 screenSize, bool debugMode)
{
	...
	for (size_t i = 0; i < images.size();  i++) {
		...
		vkGbuffer::createGbufferAttachment(physicalDevice,logicalDevice,&bundle.frames[i].gbuffer, &bundle.frames[i].particleAttachment, &bundle.frames[i].postProcessInputAttachment);
